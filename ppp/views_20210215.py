...
def period(request, *args, **kwargs):
    ...
    if len(request.POST) > 0:
        ...
        print("views.py period() tPOST oDate: ", toDate)
    ...
    process, total_process_count = makingProcess(qr_process, processCode)
    ...
    if len(df_data_period_process) == 0:  # len(df.index) == 0: 또는 df.shape[0] == 0: 같은 구문이다.
        ...
        context = {"process": process, "ppDataPeriodProcessList": [], "products": [],
...
def process_selected(request, process_code):
    ...
    ppDataPeriodProcessList, products, productsCount, ppDataPeriod = makingProduction(pp_data_period_process,
                                                                                      df_data_period_process)
...
def makingProcess(qr_process, processCode):
    if processCode == 'all':
        process = [data for data in qr_process]
    else:
        process = [data for data in qr_process if data[0] == processCode]
    ...
    return process, total_process_count
