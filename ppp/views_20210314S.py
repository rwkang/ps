...
def makingCapacity(sqlQuerySets, dfSets, revision):
    ...
    # 1. [생산 계획 수량.dpt]와 [실제 생산 소요 시간.needshour]를 [작업반명.machine_ko+코드.machine]를 기준으로 합산한다.
    #   : 혹시나 1일에 2개 설비에서 [같은 제품]을 생산한 경우에, 1개 로우로 정리한다.
    dfSetsGroupBy = dfSets.groupby(by=['machine_ko', 'machine'], as_index=False)['needshour', 'dpt'].sum().reset_index()  # df type
    ...
    # machine = [item('machine_ko') for item('machine_ko') in capacityList]
    machineList = dfSetsGroupBy['machine_ko'].tolist()
...
def makingCurrent(sqlQuerySets, dfSets, dfSetsPlanQty, revision):
    ...
    # dfSets.columns = ['revision', 'code', 'step9', 'workingdays', 'ct', 'needsday', 'needshour', 'uph',
    #                   'division', 'upd', 'stockfirst', 'dpt', 'dft', 'dmt', 'dst', 'npt', 'nft', 'nst',
    #                   'machine', 'machine_ko', 'machine_lo', 'process', 'process_ko', 'process_lo']
    ...
    dfSetsSorted = dfSets.sort_values(['workdate', 'code'])  # Default: [ascending=True]  # 위와 같은 결과...
    # print("00 makingCurrent 소트 후 dfSetsSorted: \n", dfSetsSorted)
    # print("00 makingCurrent type(dfSetsSorted): ", type(dfSetsSorted))
    ...
    dfSetsGroupBy = dfSetsSorted.groupby(by=['workdate', 'code', 'step9'], as_index=False)[['goodness', 'badness', 'productionactualno', 'processinfo']].sum()
    ...
    # print("makingCurrent goodnessSum: ", goodnessSum)
    # print("makingCurrent goodnessCumsumSum: ", goodnessCumsumSum)
    ...
    dfSetsPivotedTemp = pd.pivot_table(dfSetsCumsum, index=["workdate"], columns=["step9"],
                                       values=["goodness"], aggfunc=int).fillna(0).reset_index()
...
# 2021.03.08 Created. [누계]를 자동으로 계산해 주는 함수
def makeCumulativeColumn(df, planQtyByCodeList, revision, cumsumColumnName):
    ...
    dfSorted = df.sort_values(['workdate', 'code'], ascending=True).reset_index()
    # print("33 views.py makeCumulativeColumn() 소트 후 dfSorted: \n", dfSorted)
    # print("33 views.py makeCumulativeColumn() type(dfSorted): ", type(dfSorted))

    dfSorted = dfSorted.fillna(0)  # NaN 값을 [0]으로 치환. 반드시 [새로운 DataFrame 명] 사용...
    ...
    for i in dfSorted.index:
        ...
        # print("44 views.py makeCumulativeColumn() workdateCurrent: ", workdateCurrent)
        # print("44 views.py makeCumulativeColumn() yearCurrent: ", yearCurrent)
        # print("44 views.py makeCumulativeColumn() monthCurrent: ", monthCurrent)
        # print("44 views.py makeCumulativeColumn() dateCurrent: ", dateCurrent)
        # print("44 views.py makeCumulativeColumn() type(workdateCurrent): ", type(workdateCurrent))
        workdateCurrent = dfSorted.at[i, 'workdate'].strip()
        # print("44 views.py makeCumulativeColumn() workdateCurrent: ", workdateCurrent)
        codeCurrent = dfSorted.at[i, 'code'].strip()
        # print("44 views.py makeCumulativeColumn() codeCurrent: ", codeCurrent)
        # print("44 views.py makeCumulativeColumn() type(codeCurrent): ", type(codeCurrent))

        # 2021.03.13 Added. 생산 계획 수량을 가져와서 [dpt.planqty] 컬럼에 찍어준다.
        result = [data for data in planQtyByCodeList[:] if codeCurrent in data[:]]
        if len(result) > 0:
            # print("444 찾은 result: ", result)
            # print("444 찾은 type(result): ", type(result))
            # print("444 찾은 len(result): ", len(result))
            # print("444 찾은 result[0]: ", result[0])
            # print("444 찾은 result[0][4]: ", result[0][4])
            # print("444 찾은 type(result[0][4]): ", type(result[0][4]))
            planqtyCurrent = float(result[0][4])
            # print("444 찾은 planqtyCurrent: ", planqtyCurrent)
            # print("444 찾은 type(planqtyCurrent): ", type(planqtyCurrent))
        ...
        # print("4 찾은 planqtyList: ", planqtyList)
        # print("4 찾은 type(planqtyList): ", type(planqtyList))
