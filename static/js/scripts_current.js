...
function  setChart(processCode) {
        ...
        options: {
            legend: {
                ...
                onHover: function (event, legendItem) {
                    ...
                    // console.log("legendItem 이벤트 onHover legendItem: ", legendItem);
                },
                onClick: function (e, legendItem) {
                    var ci = this.chart;
                    var index = legendItem.datasetIndex;
                    var legendTextCurrent = ci.controller.chart.legend.legendItems[index].text; // 위의 라인과 같은 값...
                    var indexRest = index % 3; // 0: 생산 실적, 1: 생산 Volume, 2: 계획(or 불량)...
                    ...
                    if (indexRest === 0) { // legend 2개가 한쌍으로 되었을 때, 첫번째 legend 만 클릭 이벤트 반응하게 한다.
                        ...
                        var alreadyHidden2 = (ci.getDatasetMeta(index + 2).hidden === null) ? false : ci.getDatasetMeta(index + 2).hidden;
                    ...
                    // console.log("1 legendTextCurrent: ", legendTextCurrent);
    ...
    let j = 0;
    let k = 0;
    for (let i = 0; i < (graphData.length * 3); i++) {
        ...
        if (indexRest === 0) {
            ...
            // j += 1; // 2개 그래프만을 띄울 때는 여기를 활성화하고, 아래 3번째 [j += 1]를 비활성화한다.
        } else if ( indexRest === 2) {
            var newDataset = { // 생산 계획
                type: 'bar', // 2021.02.25 Conclusion. 만약 [label=legend]별로 그라프 형식을 지정하고 싶으면, 여기서 처리...
