...
// 2021.03.15 ??? 아래 import 문장은 ???
// import {LANGUAGES} from "../../ref_data/datepicker-master/src/js/constants";
...
function setChartInteractions(processCode) {
    ...
    window.onload = function () {
        ...
        // 원래는 현재 공정의 [전체 설비 수]로 계산해야 하지만, 이번 달에 아예 1도 가동하지 않는 설비는, 아예 계산에 포함하지 않는다.
        let capacity8Total = (graphData2[0][0] * productsArrList.length); // 공정 가동률
        let availability8Process = ((needsHourTotal / capacity8Total) * 100).toFixed(1); // 소숫점 1자리까지만.
        console.log("가동률 needsHourTotal: ", needsHourTotal);
        console.log("가동률 capacity8Total: ", capacity8Total);
        console.log("가동률 (needsHourTotal / capacity8Total): ", (needsHourTotal / capacity8Total));
        console.log("가동률 (needsHourTotal / capacity8Total) * 100: ", (needsHourTotal / capacity8Total) * 100);
        console.log("가동률 availability8Process: ", availability8Process);
        let availability8ProcessString = String(availability8Process) + "%";
        let monthInfoText;

        if (languageNo === 1042) {
            monthInfoText = "근무 " + String(workingdays) + "일 / " + "가동률 " + String(availability8Process) + "%";
        } else if (languageNo === 1033) {
            monthInfoText = "Working Days " + String(workingdays) + "days / " + "Operating Rate " + String(availability8Process) + "%";
        } else {
            monthInfoText = "근무 " + String(workingdays) + "일 / " + "가동률 " + String(availability8Process) + "%";
        }
...
function setChart(processCode) {
    ...
    var config = {
        ...
        options: {
            // 'onClick': graphClickEvent, 잘 안 되네...
            legend: {
                position: 'top',
                // labels: {
                ...
                onHover: function (event, legendItem) {
                    ...
                    // console.log("onHover!!!!!!!!!!!!!!")
                    // graphClickEvent; 너무 잘 되네... 숫자가 너무 많이 올라간다...
                },
                onClick: function (e, legendItem) {
                    ...
                    temIndex = temIndex + 5; // 그러므로 [코드] 중에서, [앞 5 자리]까지만 담는다.
                    legendTextCurrent = legendTextCurrent.substring(1, temIndex); // 첫문자가 작은 따옴표['] 임에 주의...
                    // console.log("legendTextCurrent: ", legendTextCurrent);
    ...
    // 2021.03.06 Added. 그래프 그리기 3단계로, 위에서 이미 그린 그래프 내부의 [config.data.datasets]을,동적으로 세팅하는 함수.
    let sof = setDynamicDataSets(config);
    console.log("setDynamicDataSets() 함수 콜 후 리턴 successOrFail: ", sof);
    ...
    if (productsArrList.length === 0) {
        document.getElementById("chartArea").style.height = 100 + "px";
        // document.getElementById("chartLegend").style.height = 300 + "px";
        document.getElementById("canvas").height = 60;
        ...
        spaceAfterChart = 10;
        document.getElementById("chartArea").style.height = 1100 + "px";
        // document.getElementById("chartLegend").style.height = 1500 + "px";
        document.getElementById("canvas").height = 330;
        // document.getElementById("canvas").height = 530;
        let chartAreaHeight = document.getElementById("chartArea").height;
        let canvasHeight = document.getElementById("canvas").height;
        ...
        options: {
            legend: {
                ...
                onClick: function (e, legendItem) {
                    ...
                    var indexFirst = index % 2; // 2로 나누든, 3으로 나누던... indexSecond...
...
function dataTableReset() {
    ...
    */
    ...
    $dataRows.each(function () {
        ...
        // // ppQtyTotal += parseInt(ppQty.html()); // 이런식으로 처리하면 에러 발생 가능성이 많다...
        currentPageTotal += parseInt(currentRowQty);
        // console.log("ppQtyTotal: ", ppQtyTotal);
        // console.log("typeof ppQtyTotal: ", typeof ppQtyTotal);
    });
    // console.log("실적 합계 ppQtyTotal: ", ppQtyTotal);
    // let ppQtyTotalString = String(ppQtyTotal);
    // document.getElementById("ppQtyTotal").innerHTML = String(ppQtyTotal);
    ...
    if (productsArrList.length > 0) {
        if (languageNo === 1042) {
            ...
            document.getElementById("processTotal").innerHTML = planQtyTotal.toLocaleString('ko-KR');
            document.getElementById("availabilityProcessTotal").innerHTML = availability8ProcessString.toLocaleString('ko-KR');
        } else if (languageNo === 1033) {
            document.getElementById("currentPageTotal").innerHTML = currentPageTotal.toLocaleString('en-US');
            document.getElementById("processTotal").innerHTML = planQtyTotal.toLocaleString('en-US');
            document.getElementById("availabilityProcessTotal").innerHTML = availability8ProcessString.toLocaleString('en-US');
        } else {
            document.getElementById("currentPageTotal").innerHTML = currentPageTotal.toLocaleString('zh-CN');
            document.getElementById("processTotal").innerHTML = planQtyTotal.toLocaleString('zh-CN');
            document.getElementById("availabilityProcessTotal").innerHTML = availability8ProcessString.toLocaleString('zh-CN');
        }
    }
